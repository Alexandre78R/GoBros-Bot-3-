...
function aide (message) {
 ...
 if(message.author.bot) return;
  ...
  let args = messageArray.slice(1);
    ...
    .setColor("#15f153")
    //.setThumbnail(sicon)
    .addField(config.prefix +"aide", "Voir les commandes du bot.")
    .addField(config.prefix +"reportmembre", "Report d'un membre du serveur // EX : !reportmembre message.")
    .addField(config.prefix +"reportbug", "Report un bug sur le discord ou sur le bot")
    .addField(config.prefix +"serverinfo", "Indique les informations du serveur.")
    .addField(config.prefix +"jeux", "Permettre de proposée vos jeux pour les streams.")
    .addField(config.prefix +"mascotte", "Pour avoir le rôle 'Apprenti Mascotte' ou le retirer.")
    .addField(config.prefix +"aide_cmd", "Permettre de vous aidez pour l'utilisation des commandes.")
    .addField(config.prefix +"avatar", "Permettre de voir votre photo de profils et avoir le lien.")
    .addField(config.prefix +"poudlard", "Permettre de vous choisir dnas une classe de Harry Potter")
    .addField(config.prefix +"fortnite", "Pour voir vos stats sur fortnite !")
    .addField(config.prefix +"gif", "Pour utilisÃ© les gifs.")
